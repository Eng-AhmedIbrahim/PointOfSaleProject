import json

# New permissions for Section 4 Actions
new_actions = [
    { "key": "CanCancelOrder", "ar": "إلغاء الطلب بالكامل", "en": "Cancel Entire Order" },
    { "key": "CanWaitingOrder", "ar": "إرسال الطلب للانتظار", "en": "Send to Waiting" },
    { "key": "CanPrintOrder", "ar": "طباعة الطلب", "en": "Print Order" }
]

def update_files():
    # 1. Update Permissions.cs
    cs_path = r'f:\PointOfSaleProject\src\POS.Authorization\Models\Permissions.cs'
    with open(cs_path, 'r', encoding='utf-8') as f:
        lines = f.readlines()
    
    new_lines = []
    found_section = False
    for line in lines:
        if "── Section 4 Actions ──" in line:
            found_section = True
        new_lines.append(line)
            
    if not found_section:
        for i in range(len(lines)-1, 0, -1):
            if "};" in lines[i]:
                insert_pos = i
                for action in new_actions:
                    lines.insert(insert_pos, f'        {{ "{action["key"]}", "{action["key"]}" }},\n')
                lines.insert(insert_pos, '        // ── Section 4 Actions ────────────────────────────────────────────────\n')
                break
        with open(cs_path, 'w', encoding='utf-8') as f:
            f.writelines(lines)

    # 2. Update permissions.json
    json_path = r'f:\PointOfSaleProject\src\Pos.Repository\Data\DataSeed\JsonFiles\permissions.json'
    with open(json_path, 'r', encoding='utf-8') as f:
        data = json.load(f)
    
    existing_names = [item["Name"] for item in data]
    for action in new_actions:
        if action["key"] not in existing_names:
            data.append({
                "Name": action["key"],
                "PoliceArabicName": action["ar"],
                "PoliceEnglishNameEn": action["en"],
                "ScreenArabicName": "أزرار تنفيذ الطلب (أطراف)",
                "ScreenEnglishName": "POS Screen Section4 Actions",
                "IsBackOffice": False
            })
    
    with open(json_path, 'w', encoding='utf-8') as f:
        json.dump(data, f, ensure_ascii=False, indent=4)

if __name__ == "__main__":
    update_files()
